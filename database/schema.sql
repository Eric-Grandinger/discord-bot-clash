CREATE TABLE IF NOT EXISTS clashDateCache(
  ...
  notifiedUsers INTEGER NOT NULL CHECK(clashDateCache.notifiedUsers IN(0,1))
);
CREATE TABLE IF NOT EXISTS clashDateCacheSchedule(
  idOfClash INTEGER PRIMARY KEY,
  registrationTime INTEGER NOT NULL,
  startTime INTEGER NOT NULL,
  cancelled INTEGER NOT NULL CHECK(clashDateCacheSchedule.cancelled IN(0,1)),
  scheduleSet INTEGER NOT NULL CHECK(clashDateCacheSchedule.cancelled IN(0,1)),
